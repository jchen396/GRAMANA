...
const PlayScreen = () => {
  ...
  const tileRefs = useRef<any>([...new Array(5)].map(() => React.createRef()));
  ...
  useEffect(() => {
    ...
    socket.on("start", (userList, RANDOM_ANAGRAM_LIST: string[], playerObj) => {
      setPlayerTurn(userList[0].id);
      if (playerObj.hasOwnProperty(socket.id)) {
        setPlayerName(userList[playerObj[socket.id]].name);
        setWord(RANDOM_ANAGRAM_LIST[playerObj[socket.id]].toUpperCase());
        setAvaialbleLetters(
          RANDOM_ANAGRAM_LIST[playerObj[socket.id]].toUpperCase().split("")
        );
      }
      socket.emit("turn", userList[0].id);
  ...
  useEffect(() => {
    ...
    const checkDirection = (prevTile: number, nextTile: number): void => {
      if (checkWin === false) {
        ...
        let leftCharacters = availableLetters.indexOf(tiles[selectedIndex]);
        ...
        if (availableLetters.indexOf(tiles[selectedIndex], 2) !== -1) {
          ...
          if (rightCharacters === 0) rightCheck = true;
            ...
            for (let i = 1; i < rightCharacters + 1; i++) {
              ...
              if (
                availableLetters[
                  availableLetters.indexOf(tiles[selectedIndex], 2) + i
                ] === tiles[selectedIndex + i * nextTile]
              ) {
                if (i === rightCharacters || rightCharacters === 0) {
                  rightCheck = true;
                }
                continue;
              } else break;
  ...
  useEffect(() => {
    //check game status
    if (isWin) socket.emit("result", word, playerName, currentScore, socket.id);
  }, [isWin]);
  ...
  const selectTile = (key: number, divEvent: any) => {
    if (gameStart && playerTurn == socket.id && showResult === false) {
      ...
      const pressInputTile = (keyPressEvent: any) => {
        let inputChar;
        if (availableLetters.includes(keyPressEvent.key.toUpperCase())) {
  ...
  return (
    <>
      <div className=" relative mt-20 m-4 sm:m-10 sm:p-10 p-2 w-screen sm:w-full lg:w-1/2 md:3/4 sm:h-4/5 h-1/2 bg-neutral-800 rounded-2xl place-content-center place-items-center">
        <div className=" ">
          <GameHeader
            socketId={socket.id}
        ...
        <br />
        <div className="sm:m-10 grid grid-cols-8 sm:gap-y-4 md:gap-x-2">
          {tiles.map((value, key) => (
            <div
